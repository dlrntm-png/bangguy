...
async function ensureTables() {
  ...
  await db.query(`
    CREATE INDEX IF NOT EXISTS idx_allowed_ips_ip_cidr
      ON allowed_ips (ip_cidr)
  `);
...
export async function getAllowedIps() {
  if (SHOULD_USE_MOCK) {
    return mockData.allowedIps.map((ip) => ({ ...ip }));
  }

  await ensureTables();
  const db = getPool();
  const { rows } = await db.query(
    `SELECT id, ip_cidr, description, created_at, created_by
     FROM allowed_ips
     ORDER BY created_at DESC`
  );
  return rows;
...
export async function addAllowedIp(ipCidr, description = null, createdBy = null) {
  if (SHOULD_USE_MOCK) {
    const existing = mockData.allowedIps.find((ip) => ip.ip_cidr === ipCidr);
    if (existing) {
      throw new Error('이미 등록된 IP입니다.');
    }
    const newIp = {
      id: mockData.allowedIps.length + 1,
      ip_cidr: ipCidr,
      description: description || null,
      created_at: new Date().toISOString(),
      created_by: createdBy || null
    };
    mockData.allowedIps.push(newIp);
    return newIp;
  }
  ...
  try {
    const { rows } = await db.query(
      `INSERT INTO allowed_ips (ip_cidr, description, created_by)
       VALUES ($1, $2, $3)
       RETURNING id, ip_cidr, description, created_at, created_by`,
      [ipCidr, description, createdBy]
    );
    return rows[0];
  } catch (err) {
...
export async function removeAllowedIp(id) {
  if (SHOULD_USE_MOCK) {
    const index = mockData.allowedIps.findIndex((ip) => ip.id === id);
    if (index === -1) {
      throw new Error('IP를 찾을 수 없습니다.');
    }
    mockData.allowedIps.splice(index, 1);
    return true;
  }
