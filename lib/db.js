...
export async function getConsentLogsFromDb() {
  ...
  return rows.map((row) => ({
    ...
    consentedAt: row.consented_at ? row.consented_at.toISOString ? row.consented_at.toISOString() : row.consented_at : null,
    blobPath: row.blob_path,
    blobSize: row.blob_size,
    uploadedAt: row.uploaded_at ? row.uploaded_at.toISOString ? row.uploaded_at.toISOString() : row.uploaded_at : null
  }));
}
