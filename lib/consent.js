...
import { uploadTextContent, blobExists, listBlobs, readBlobText } from './blob.js';
import {
  hasConsentLogEntry,
  saveConsentLogEntry,
...
async function syncConsentLogFromBlob(pathname, defaults = {}) {
  try {
    ...
    if (err?.code === 'BLOB_NOT_FOUND') {
      console.warn('[consent] Blob missing during sync, skipping:', pathname);
      return null;
    }
...
export async function hasConsentLog(employeeId) {
  ...
  try {
    ...
    console.warn('[consent] hasConsentLogEntry check failed:', err?.message || err);
  }
  ...
  const synced = await syncConsentLogFromBlob(pathname, { employeeId });
  if (synced) {
    return true;
  }
  return false;
